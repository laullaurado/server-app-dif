...
define(["require", "exports", "assert", "vs/editor/common/core/wordCharacterClassifier", "vs/editor/common/core/position", "vs/editor/common/core/range", "vs/editor/common/model", "vs/editor/common/model/pieceTreeTextBuffer/pieceTreeTextBufferBuilder", "vs/editor/common/model/pieceTreeTextBuffer/rbTreeBase", "vs/editor/test/common/testTextModel", "vs/base/common/strings"], function (require, exports, assert, wordCharacterClassifier_1, position_1, range_1, model_1, pieceTreeTextBufferBuilder_1, rbTreeBase_1, testTextModel_1, strings_1) {
    ...
    function testLinesContent(str, pieceTable) {
        let lines = (0, strings_1.splitLines)(str);
    ...
    function createTextBuffer(val, normalizeEOL = true) {
        let bufferBuilder = new pieceTreeTextBufferBuilder_1.PieceTreeTextBufferBuilder();
        for (const chunk of val) {
            bufferBuilder.acceptChunk(chunk);
        }
        let factory = bufferBuilder.finish(normalizeEOL);
        return factory.create(1 /* DefaultEndOfLine.LF */).textBuffer.getPieceTree();
    }
    ...
    suite('chunk based search', () => {
        test('#45892. For some cases, the buffer is empty but we still try to search', () => {
            ...
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(1, 1, 1, 1), new model_1.SearchData(/abc/, new wordCharacterClassifier_1.WordCharacterClassifier(',./'), 'abc'), true, 1000);
        ...
        test('#45770. FindInNode should not cross node boundary.', () => {
            ...
            pieceTree.insert(16, ' ');
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(1, 1, 4, 13), new model_1.SearchData(/\[/gi, new wordCharacterClassifier_1.WordCharacterClassifier(',./'), '['), true, 1000);
        ...
        test('search searching from the middle', () => {
            ...
            let ret = pieceTree.findMatchesLineByLine(new range_1.Range(2, 3, 2, 6), new model_1.SearchData(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
            assert.deepStrictEqual(ret[0].range, new range_1.Range(2, 3, 2, 4));
            pieceTree.delete(4, 1);
            ret = pieceTree.findMatchesLineByLine(new range_1.Range(2, 2, 2, 5), new model_1.SearchData(/a/gi, null, 'a'), true, 1000);
            assert.strictEqual(ret.length, 1);
