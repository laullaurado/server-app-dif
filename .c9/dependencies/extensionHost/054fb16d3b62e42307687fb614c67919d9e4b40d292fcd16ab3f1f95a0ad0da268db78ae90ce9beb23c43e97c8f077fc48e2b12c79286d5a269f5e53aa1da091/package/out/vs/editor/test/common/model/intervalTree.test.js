...
define(["require", "exports", "assert", "vs/editor/common/model/intervalTree"], function (require, exports, assert, intervalTree_1) {
    ...
    suite('IntervalTree', () => {
        ...
        suite('searching', () => {
            function createCormenTree() {
                let r = new intervalTree_1.IntervalTree();
                let data = [
                    [16, 21],
                    [8, 9],
                    [25, 30],
                ...
                data.forEach((int) => {
                    let node = new intervalTree_1.IntervalNode(null, int[0], int[1]);
                    r.insert(node);
                });
                return r;
            }
            ...
            function assertIntervalSearch(start, end, expected) {
                ...
                let actual = actualNodes.map((n) => [n.cachedAbsoluteStart, n.cachedAbsoluteEnd]);
                assert.deepStrictEqual(actual, expected);
            }
            test('cormen 1->2', () => {
    ...
    suite('IntervalTree', () => {
        function assertNodeAcceptEdit(msg, nodeStart, nodeEnd, nodeStickiness, start, end, textLength, forceMoveMarkers, expectedNodeStart, expectedNodeEnd) {
            let node = new intervalTree_1.IntervalNode('', nodeStart, nodeEnd);
