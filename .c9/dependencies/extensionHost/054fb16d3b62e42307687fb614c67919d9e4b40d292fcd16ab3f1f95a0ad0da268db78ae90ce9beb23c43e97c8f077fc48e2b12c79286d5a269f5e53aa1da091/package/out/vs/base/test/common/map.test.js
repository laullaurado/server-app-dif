 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/arrays", "vs/base/common/extpath", "vs/base/common/map", "vs/base/common/resources", "vs/base/common/stopwatch", "vs/base/common/uri"], function (require, exports, assert, arrays_1, extpath_1, map_1, resources_1, stopwatch_1, uri_1) {
    "use strict";
    ...
    suite('Map', () => {
        ...
        test('URIIterator', function () {
            const iter = new map_1.UriIterator(() => false, () => false);
        ...
        test('URIIterator - ignore query/fragment', function () {
            const iter = new map_1.UriIterator(() => false, () => true);
            iter.reset(uri_1.URI.parse('file:///usr/bin/file.txt'));
            assert.strictEqual(iter.value(), 'file');
            // assert.strictEqual(iter.cmp('FILE'), 0);
            assert.strictEqual(iter.cmp('file'), 0);
        ...
        test('TernarySearchTree: Cannot read property \'1\' of undefined #138284', function () {
            ...
            const tst = map_1.TernarySearchTree.forUris();
        ...
        test('TernarySearchTree: Cannot read property \'1\' of undefined #138284 (simple)', function () {
            const keys = ['C', 'A', 'D', 'B',];
            const tst = map_1.TernarySearchTree.forStrings();
            for (let item of keys) {
                tst.set(item, true);
            }
        ...
        test('TernarySearchTree (URI) - basics', function () {
            let trie = new map_1.TernarySearchTree(new map_1.UriIterator(() => false, () => false));
        ...
        test('TernarySearchTree (URI) - query parameters', function () {
            let trie = new map_1.TernarySearchTree(new map_1.UriIterator(() => false, () => true));
        ...
        test('TernarySearchTree (URI) - lookup', function () {
            const map = new map_1.TernarySearchTree(new map_1.UriIterator(() => false, () => false));
        ...
        test('TernarySearchTree (URI) - lookup, casing', function () {
            const map = new map_1.TernarySearchTree(new map_1.UriIterator(uri => /^https?$/.test(uri.scheme), () => false));
            map.set(uri_1.URI.parse('http://foo.bar/user/foo/bar'), 1);
            assert.strictEqual(map.get(uri_1.URI.parse('http://foo.bar/USER/foo/bar')), 1);
