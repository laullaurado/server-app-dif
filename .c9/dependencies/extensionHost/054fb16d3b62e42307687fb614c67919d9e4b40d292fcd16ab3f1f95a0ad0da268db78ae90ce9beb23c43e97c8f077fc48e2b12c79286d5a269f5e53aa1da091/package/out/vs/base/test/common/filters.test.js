define(["require", "exports", "assert", "vs/base/common/filters"], function (require, exports, assert, filters_1) {
    ...
    suite('Filters', () => {
        ...
        test('WordFilter', () => {
            ...
            filterOk(filters_1.matchesWords, 'git プル', 'git: プル', [{ start: 0, end: 4 }, { start: 5, end: 7 }]);
        ...
        function assertMatches(pattern, word, decoratedWord, filter, opts = {}) {
            var _a;
            let r = filter(pattern, pattern.toLowerCase(), opts.patternPos || 0, word, word.toLowerCase(), opts.wordPos || 0, { firstMatchCanBeWeak: (_a = opts.firstMatchCanBeWeak) !== null && _a !== void 0 ? _a : false, boostFullMatch: true });
            assert.ok(!decoratedWord === !r);
            if (r) {
                let matches = (0, filters_1.createMatches)(r);
                let actualWord = '';
                let pos = 0;
        ...
        test('configurable full match boost', function () {
            ...
            let b = 'create';
            let aBoost = (0, filters_1.fuzzyScore)(prefix, prefix, 0, a, a.toLowerCase(), 0, { boostFullMatch: true, firstMatchCanBeWeak: true });
            let bBoost = (0, filters_1.fuzzyScore)(prefix, prefix, 0, b, b.toLowerCase(), 0, { boostFullMatch: true, firstMatchCanBeWeak: true });
            assert.ok(aBoost);
            assert.ok(bBoost);
            assert.ok(aBoost[0] < bBoost[0]);
            let aScore = (0, filters_1.fuzzyScore)(prefix, prefix, 0, a, a.toLowerCase(), 0, { boostFullMatch: false, firstMatchCanBeWeak: true });
            let bScore = (0, filters_1.fuzzyScore)(prefix, prefix, 0, b, b.toLowerCase(), 0, { boostFullMatch: false, firstMatchCanBeWeak: true });
            assert.ok(aScore);
            assert.ok(bScore);
