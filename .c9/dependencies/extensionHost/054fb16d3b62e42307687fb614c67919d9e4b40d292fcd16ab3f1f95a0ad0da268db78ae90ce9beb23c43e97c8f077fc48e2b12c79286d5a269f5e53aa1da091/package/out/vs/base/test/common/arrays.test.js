define(["require", "exports", "assert", "vs/base/common/arrays"], function (require, exports, assert, arrays) {
    ...
    suite('Arrays', () => {
        ...
        test('findMaxBy', () => {
            ...
            assert.strictEqual(array.indexOf(arrays.findMaxBy(array, arrays.compareBy(v => v.v, arrays.numberComparator))), 1);
        });
        test('findLastMaxBy', () => {
            const array = [{ v: 3 }, { v: 5 }, { v: 2 }, { v: 2 }, { v: 2 }, { v: 5 }];
            assert.strictEqual(array.indexOf(arrays.findLastMaxBy(array, arrays.compareBy(v => v.v, arrays.numberComparator))), 5);
        });
        test('findMinBy', () => {
            const array = [{ v: 3 }, { v: 5 }, { v: 2 }, { v: 2 }, { v: 2 }, { v: 5 }];
            assert.strictEqual(array.indexOf(arrays.findMinBy(array, arrays.compareBy(v => v.v, arrays.numberComparator))), 2);
        });
        suite('ArrayQueue', () => {
