...
define(["require", "exports", "vs/base/common/errors", "vs/platform/extensions/common/extensions", "vs/workbench/services/extensions/common/extensions", "vs/platform/log/common/log", "vs/base/common/async"], function (require, exports, errors, extensions_1, extensions_2, log_1, async_1) {
    "use strict";
    ...
    let ExtensionsActivator = class ExtensionsActivator {
        constructor(registry, resolvedExtensions, externalExtensions, host, _logService) {
            ...
            externalExtensions.forEach((extensionId) => this._externalExtensionsMap.set(extensions_1.ExtensionIdentifier.toKey(extensionId), extensionId));
            this._host = host;
        ...
        isActivated(extensionId) {
            const op = this._operations.get(extensions_1.ExtensionIdentifier.toKey(extensionId));
        ...
        _handleActivationRequest(currentActivation) {
            if (this._operations.has(extensions_1.ExtensionIdentifier.toKey(currentActivation.id))) {
                return this._operations.get(extensions_1.ExtensionIdentifier.toKey(currentActivation.id));
            }
            if (this._externalExtensionsMap.has(extensions_1.ExtensionIdentifier.toKey(currentActivation.id))) {
            ...
            for (const depId of depIds) {
                ...
                const dep = this._operations.get(extensions_1.ExtensionIdentifier.toKey(depId));
                if (dep) {
                    deps.push(dep);
                    continue;
                }
                if (this._externalExtensionsMap.has(extensions_1.ExtensionIdentifier.toKey(depId))) {
                    // must first wait for the dependency to activate
                    deps.push(this._handleActivationRequest({
                        id: this._externalExtensionsMap.get(extensions_1.ExtensionIdentifier.toKey(depId)),
        ...
        _createAndSaveOperation(activation, displayName, deps, value) {
            ...
            this._operations.set(extensions_1.ExtensionIdentifier.toKey(activation.id), operation);
            return operation;
        }
    };
