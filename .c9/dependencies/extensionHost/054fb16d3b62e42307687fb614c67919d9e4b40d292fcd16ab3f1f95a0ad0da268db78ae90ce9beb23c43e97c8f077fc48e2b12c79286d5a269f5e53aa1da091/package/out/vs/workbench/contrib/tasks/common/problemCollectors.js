        ...
        constructor(problemMatchers, markerService, modelService, fileService) {
            ...
            this.modelListeners.add(this.modelService.onModelRemoved(modelEvent => {
                let markerChanged = event_1.Event.debounce(this.markerService.onMarkerChanged, (last, e) => {
                    return (last !== null && last !== void 0 ? last : []).concat(e);
                }, 500)(async (markerEvent) => {
                    markerChanged === null || markerChanged === void 0 ? void 0 : markerChanged.dispose();
        ...
        aboutToStart() {
            for (let background of this.backgroundPatterns) {
                if (background.matcher.watching && background.matcher.watching.activeOnStart) {
                    ...
                    this._onDidStateChange.fire(ProblemCollectorEvent.create("backgroundProcessingBegins" /* ProblemCollectorEventKind.BackgroundProcessingBegins */));
        ...
        async tryBegin(line) {
            ...
            for (const background of this.backgroundPatterns) {
                ...
                if (matches) {
                    ...
                    if (file) {
                        let resource = (0, problemMatcher_1.getResource)(file, background.matcher);
            ...
            for (const background of this.backgroundPatterns) {
                ...
                if (matches) {
                    if (this._activeBackgroundMatchers.has(background.key)) {
                        ...
                        this.resetCurrentResource();
                        this._onDidStateChange.fire(ProblemCollectorEvent.create("backgroundProcessingEnds" /* ProblemCollectorEventKind.BackgroundProcessingEnds */));
                        result = true;
                        this.lines.push(line);
                        let owner = background.matcher.owner;
