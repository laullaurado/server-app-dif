...
define(["require", "exports", "assert", "vs/workbench/contrib/snippets/browser/snippetsFile"], function (require, exports, assert, snippetsFile_1) {
    ...
    suite('SnippetRewrite', function () {
        function assertRewrite(input, expected) {
            const actual = new snippetsFile_1.Snippet(['foo'], 'foo', 'foo', 'foo', input, 'foo', 1 /* SnippetSource.User */);
        ...
        test('lazy bogous variable rewrite', function () {
            const snippet = new snippetsFile_1.Snippet(['fooLang'], 'foo', 'prefix', 'desc', 'This is ${bogous} because it is a ${var}', 'source', 3 /* SnippetSource.Extension */);
