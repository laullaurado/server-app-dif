 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/workbench/services/textfile/common/textfiles", "vs/base/common/uri", "vs/base/common/path", "vs/workbench/services/textfile/common/encoding", "vs/base/common/buffer", "vs/editor/test/common/testTextModel", "vs/base/common/platform", "vs/editor/common/model/textModel"], function (require, exports, assert, textfiles_1, uri_1, path_1, encoding_1, buffer_1, testTextModel_1, platform_1, textModel_1) {
    "use strict";
    ...
    function createSuite(params) {
        ...
        function createTextModelSnapshot(text, preserveBOM) {
            const textModel = (0, testTextModel_1.createTextModel)(text);
        ...
        async function testEncoding(resource, encoding, content, expectedContent) {
            ...
            assert.strictEqual((0, textfiles_1.snapshotToString)(resolved.value.create(platform_1.isWindows ? 2 /* DefaultEndOfLine.CRLF */ : 1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false)), expectedContent);
        }
        ...
        async function testEncodingKeepsData(resource, encoding, expected) {
            ...
            const content = (0, textfiles_1.snapshotToString)(resolved.value.create(platform_1.isWindows ? 2 /* DefaultEndOfLine.CRLF */ : 1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
            assert.strictEqual(content, expected);
            await service.write(resource, content, { encoding });
            resolved = await service.readStream(resource, { encoding });
            assert.strictEqual((0, textfiles_1.snapshotToString)(resolved.value.create(2 /* DefaultEndOfLine.CRLF */).textBuffer.createSnapshot(false)), content);
        ...
        test('write - UTF8 variations - content as snapshot', async () => {
            ...
            let detectedEncoding = await detectEncodingByBOM(resource.fsPath);
            assert.strictEqual(detectedEncoding, null);
            const model = (0, testTextModel_1.createTextModel)((await readFile(resource.fsPath)).toString() + 'updates');
        ...
        async function testReadStream(resource) {
            ...
            const content = (await readFile(resource.fsPath)).toString();
            assert.strictEqual((0, textfiles_1.snapshotToString)(result.value.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false)), (0, textfiles_1.snapshotToString)(createTextModelSnapshot(content, false)));
        }
        ...
        async function testLargeEncoding(encoding, needle) {
            ...
            const result = await service.readStream(resource, { encoding });
            assert.strictEqual(result.encoding, encoding);
            let contents = (0, textfiles_1.snapshotToString)(result.value.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
            ...
            contents = (0, textfiles_1.snapshotToString)(factory.create(1 /* DefaultEndOfLine.LF */).textBuffer.createSnapshot(false));
            assert.strictEqual(contents.indexOf(needle), 0);
            assert.ok(contents.indexOf(needle, 10) > 0);
        }
