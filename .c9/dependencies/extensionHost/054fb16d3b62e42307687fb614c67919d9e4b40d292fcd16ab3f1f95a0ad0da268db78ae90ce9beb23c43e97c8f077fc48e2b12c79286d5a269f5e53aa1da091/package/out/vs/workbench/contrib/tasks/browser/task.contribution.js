    ...
    let TaskStatusBarContributions = class TaskStatusBarContributions extends lifecycle_1.Disposable {
        ...
        registerListeners() {
            ...
            this.taskService.onDidStateChange(event => {
                ...
                if (promise && (event.kind === "active" /* TaskEventKind.Active */) && (this.activeTasksCount === 1)) {
                    this.progressService.withProgress({ location: 10 /* ProgressLocation.Window */, command: 'workbench.action.tasks.showTasks' }, progress => {
        ...
        async updateRunningTasksStatus() {
            ...
            if (tasks.length === 0) {
                ...
                if (!this.runningTasksStatusItem) {
                    this.runningTasksStatusItem = this.statusbarService.addEntry(itemProps, 'status.runningTasks', 0 /* StatusbarAlignment.LEFT */, 49 /* Medium Priority, next to Markers */);
                }
                else {
        ...
        ignoreEventForUpdateRunningTasksCount(event) {
            ...
            if (((0, types_1.isString)(event.group) ? event.group : (_a = event.group) === null || _a === void 0 ? void 0 : _a._id) !== tasks_1.TaskGroup.Build._id) {
                return true;
            }
            if (!event.__task) {
                return false;
