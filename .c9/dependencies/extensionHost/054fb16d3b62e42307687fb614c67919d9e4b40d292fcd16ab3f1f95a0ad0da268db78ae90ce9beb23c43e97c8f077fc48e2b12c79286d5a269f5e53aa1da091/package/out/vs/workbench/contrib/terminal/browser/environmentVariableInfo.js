...
define(["require", "exports", "vs/workbench/contrib/terminal/common/environmentVariable", "vs/workbench/contrib/terminal/browser/terminal", "vs/nls", "vs/base/common/codicons"], function (require, exports, environmentVariable_1, terminal_1, nls_1, codicons_1) {
    "use strict";
    ...
    let EnvironmentVariableInfoStale = class EnvironmentVariableInfoStale {
        ...
        getInfo() {
            ...
            if (addsAndChanges.length > 0) {
                info = (0, nls_1.localize)('extensionEnvironmentContributionChanges', "Extensions want to make the following changes to the terminal's environment:");
                info += '\n\n';
                info += '```\n';
            ...
            if (removals.length > 0) {
                info += info.length > 0 ? '\n\n' : '';
                info += (0, nls_1.localize)('extensionEnvironmentContributionRemoval', "Extensions want to remove these existing changes from the terminal's environment:");
                info += '\n\n';
                info += '```\n';
                info += removals.join('\n');
                info += '\n```';
    ...
    class EnvironmentVariableInfoChangesActive {
        ...
        getInfo() {
            ...
            const message = (0, nls_1.localize)('extensionEnvironmentContributionInfo', "Extensions have made changes to this terminal's environment");
            return message + '\n\n```\n' + changes.join('\n') + '\n```';
        }
        getIcon() {
