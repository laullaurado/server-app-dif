 ...
 *--------------------------------------------------------------------------------------------*/
define(["require", "exports", "assert", "vs/base/common/lifecycle", "vs/base/common/network", "vs/base/parts/storage/common/storage", "vs/base/test/common/testUtils", "vs/base/test/common/timeTravelScheduler", "vs/platform/files/common/fileService", "vs/platform/files/common/inMemoryFilesystemProvider", "vs/platform/log/common/log", "vs/platform/storage/browser/storageService", "vs/platform/storage/test/common/storageService.test"], function (require, exports, assert_1, lifecycle_1, network_1, storage_1, testUtils_1, timeTravelScheduler_1, fileService_1, inMemoryFilesystemProvider_1, log_1, storageService_1, storageService_test_1) {
    "use strict";
    ...
    async function createStorageService() {
        ...
        const storageService = disposables.add(new storageService_1.BrowserStorageService({ id: 'workspace-storage-test' }, logService));
    ...
    (0, testUtils_1.flakySuite)('StorageService (browser specific)', () => {
        ...
        test('clear', () => {
            return (0, timeTravelScheduler_1.runWithFakedTimers)({ useFakeTimers: true }, async () => {
                storageService.store('bar', 'foo', 0 /* StorageScope.GLOBAL */, 1 /* StorageTarget.MACHINE */);
                storageService.store('bar', 3, 0 /* StorageScope.GLOBAL */, 0 /* StorageTarget.USER */);
    ...
    (0, testUtils_1.flakySuite)('IndexDBStorageDatabase (browser)', () => {
        ...
        const logService = new log_1.NullLogService();
        teardown(async () => {
            const storage = await storageService_1.IndexedDBStorageDatabase.create({ id }, logService);
            await storage.clear();
        });
        test('Basics', async () => {
            let storage = new storage_1.Storage(await storageService_1.IndexedDBStorageDatabase.create({ id }, logService));
            await storage.init();
            // Insert initial data
            storage.set('bar', 'foo');
            ...
            await storage.close();
            storage = new storage_1.Storage(await storageService_1.IndexedDBStorageDatabase.create({ id }, logService));
            await storage.init();
        ...
        test('Clear', async () => {
            ...
            storage.set('barBoolean', true);
            await storage.close();
            const db = await storageService_1.IndexedDBStorageDatabase.create({ id }, logService);
            storage = new storage_1.Storage(db);
            await storage.init();
            await db.clear();
            storage = new storage_1.Storage(await storageService_1.IndexedDBStorageDatabase.create({ id }, logService));
            await storage.init();
