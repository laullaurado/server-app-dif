...
suite('git smoke test', function () {
    ...
    test('reflects working tree changes', async function () {
        ...
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.uri.path && r.status === 5 /* Status.MODIFIED */);
        ...
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.uri.path && r.status === 5 /* Status.MODIFIED */);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.uri.path && r.status === 7 /* Status.UNTRACKED */);
    });
    test('opens diff editor', async function () {
        ...
        assert.strictEqual(vscode_1.window.activeTextEditor.document.uri.path, appjs.path);
    ...
    test('stages correctly', async function () {
        ...
        assert.strictEqual(repository.state.workingTreeChanges.length, 1);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.path && r.status === 7 /* Status.UNTRACKED */);
        assert.strictEqual(repository.state.indexChanges.length, 1);
        repository.state.indexChanges.some(r => r.uri.path === appjs.path && r.status === 0 /* Status.INDEX_MODIFIED */);
        await vscode_1.commands.executeCommand('git.unstage', appjs);
        assert.strictEqual(repository.state.workingTreeChanges.length, 2);
        repository.state.workingTreeChanges.some(r => r.uri.path === appjs.path && r.status === 5 /* Status.MODIFIED */);
        repository.state.workingTreeChanges.some(r => r.uri.path === newfile.path && r.status === 7 /* Status.UNTRACKED */);
    });
    ...
    test('rename/delete conflict', async function () {
        ...
        setTimeout(() => {
            vscode_1.commands.executeCommand('workbench.scm.focus');
        }, 2e3);
