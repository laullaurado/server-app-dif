...
async function categorizeResourceByResolution(resources) {
    ...
    const isBothAddedOrModified = (s) => s.type === 16 /* Status.BOTH_MODIFIED */ || s.type === 14 /* Status.BOTH_ADDED */;
    const isAnyDeleted = (s) => s.type === 13 /* Status.DELETED_BY_THEM */ || s.type === 12 /* Status.DELETED_BY_US */;
...
function getCheckoutProcessor(type) {
    switch (type) {
        case 'local':
            return new CheckoutProcessor(0 /* RefType.Head */, CheckoutItem);
        case 'remote':
            return new CheckoutProcessor(1 /* RefType.RemoteHead */, CheckoutRemoteHeadItem);
        case 'tags':
            return new CheckoutProcessor(2 /* RefType.Tag */, CheckoutTagItem);
    }
    return undefined;
}
...
class CommandCenter {
    constructor(git, model, outputChannelLogger, telemetryReporter) {
        this.git = git;
        this.model = model;
    ...
    async openChanges(repository) {
        for (const resource of [...repository.workingTreeGroup.resourceStates, ...repository.untrackedGroup.resourceStates]) {
            if (resource.type === 6 /* Status.DELETED */ || resource.type === 13 /* Status.DELETED_BY_THEM */ ||
                resource.type === 12 /* Status.DELETED_BY_US */ || resource.type === 15 /* Status.BOTH_DELETED */) {
                continue;
            }
    ...
    async openMergeEditor(uri) {
        ...
        let input1 = { uri: mergeUris.ours };
        let input2 = { uri: mergeUris.theirs };
        try {
    ...
    async openFile(arg, ...resourceStates) {
        ...
        const previousVisibleRange = activeTextEditor === null || activeTextEditor === void 0 ? void 0 : activeTextEditor.visibleRanges[0];
        const previousURI = activeTextEditor === null || activeTextEditor === void 0 ? void 0 : activeTextEditor.document.uri;
        const previousSelection = activeTextEditor === null || activeTextEditor === void 0 ? void 0 : activeTextEditor.selection;
        for (const uri of uris) {
            ...
            await vscode_1.commands.executeCommand('vscode.open', uri, {
                ...opts,
                override: arg instanceof repository_1.Resource && arg.type === 16 /* Status.BOTH_MODIFIED */ ? false : undefined
            });
            ...
            if ((document === null || document === void 0 ? void 0 : document.uri.toString()) !== uri.toString() || !activeTextEditor || !previousURI || !previousSelection) {
                continue;
            }
    ...
    async _stageDeletionConflict(repository, uri) {
        ...
        if (resource.type === 13 /* Status.DELETED_BY_THEM */) {
        ...
        else if (resource.type === 12 /* Status.DELETED_BY_US */) {
    ...
    async acceptMerge(uri) {
        ...
        const doc = vscode_1.workspace.textDocuments.find(doc => doc.uri.toString() === uri.toString());
        if (!doc) {
        ...
        if (activeTab && (activeTab === null || activeTab === void 0 ? void 0 : activeTab.input) instanceof vscode_1.TabInputText && activeTab.input.uri.toString() === uri.toString()) {
            await vscode_1.window.tabGroups.close(activeTab, true);
        }
    }
    ...
    async smartCommit(repository, getCommitMessage, opts) {
        ...
        // Branch protection
        const branchProtection = config.get('branchProtection').map(bp => bp.trim()).filter(bp => bp !== '');
        const branchProtectionPrompt = config.get('branchProtectionPrompt');
        const branchIsProtected = branchProtection.some(bp => { var _a, _b; return picomatch.isMatch((_b = (_a = repository.HEAD) === null || _a === void 0 ? void 0 : _a.name) !== null && _b !== void 0 ? _b : '', bp); });
    ...
    generateRandomBranchName(repository, separator) {
        const config = vscode_1.workspace.getConfiguration('git');
        ...
        for (let index = 0; index < 5; index++) {
            ...
            if (!repository.refs.find(r => r.type === 0 /* RefType.Head */ && r.name === randomName)) {
                return randomName;
            }
        }
        return '';
    ...
    async deleteBranch(repository, name, force) {
        ...
        if (typeof name === 'string') {
            ...
            const heads = repository.refs.filter(ref => ref.type === 0 /* RefType.Head */ && ref.name !== currentHead)
    ...
    async merge(repository) {
        ...
        const heads = repository.refs.filter(ref => ref.type === 0 /* RefType.Head */)
            .filter(ref => ref.name || ref.commit)
            .map(ref => new MergeItem(ref));
        const remoteHeads = (includeRemotes ? repository.refs.filter(ref => ref.type === 1 /* RefType.RemoteHead */) : [])
            .filter(ref => ref.name || ref.commit)
            .map(ref => new MergeItem(ref));
    ...
    async deleteTag(repository) {
        const picks = repository.refs.filter(ref => ref.type === 2 /* RefType.Tag */)
    ...
    async stashDropAll(repository) {
        ...
        const result = await vscode_1.window.showWarningMessage(question, yes);
        if (result !== yes) {
            return;
        }
            ...
            return result.catch(async (err) => {
                ...
                switch (err.gitErrorCode) {
                    ...
                    case "NoUserEmailConfigured" /* GitErrorCodes.NoUserEmailConfigured */:
                        ...
                        choices.set(localize('learn more', "Learn More"), () => vscode_1.commands.executeCommand('vscode.open', vscode_1.Uri.parse('https://aka.ms/vscode-setup-git')));
                        break;
                    default: {
