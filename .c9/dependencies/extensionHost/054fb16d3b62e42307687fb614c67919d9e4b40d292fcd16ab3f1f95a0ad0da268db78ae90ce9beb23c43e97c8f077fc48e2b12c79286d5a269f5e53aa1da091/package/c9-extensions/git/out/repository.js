...
class Resource {
    ...
    static getStatusText(type) {
        switch (type) {
            case 0 /* Status.INDEX_MODIFIED */: return localize('index modified', "Index Modified");
            case 5 /* Status.MODIFIED */: return localize('modified', "Modified");
            case 1 /* Status.INDEX_ADDED */: return localize('index added', "Index Added");
            case 2 /* Status.INDEX_DELETED */: return localize('index deleted', "Index Deleted");
            case 6 /* Status.DELETED */: return localize('deleted', "Deleted");
            case 3 /* Status.INDEX_RENAMED */: return localize('index renamed', "Index Renamed");
            case 4 /* Status.INDEX_COPIED */: return localize('index copied', "Index Copied");
            case 7 /* Status.UNTRACKED */: return localize('untracked', "Untracked");
            case 8 /* Status.IGNORED */: return localize('ignored', "Ignored");
            case 9 /* Status.INTENT_TO_ADD */: return localize('intent to add', "Intent to Add");
            case 15 /* Status.BOTH_DELETED */: return localize('both deleted', "Conflict: Both Deleted");
            case 10 /* Status.ADDED_BY_US */: return localize('added by us', "Conflict: Added By Us");
            case 13 /* Status.DELETED_BY_THEM */: return localize('deleted by them', "Conflict: Deleted By Them");
            case 11 /* Status.ADDED_BY_THEM */: return localize('added by them', "Conflict: Added By Them");
            case 12 /* Status.DELETED_BY_US */: return localize('deleted by us', "Conflict: Deleted By Us");
            case 14 /* Status.BOTH_ADDED */: return localize('both added', "Conflict: Both Added");
            case 16 /* Status.BOTH_MODIFIED */: return localize('both modified', "Conflict: Both Modified");
            default: return '';
        }
    }
    get resourceUri() {
        if (this.renameResourceUri && (this._type === 5 /* Status.MODIFIED */ || this._type === 6 /* Status.DELETED */ || this._type === 3 /* Status.INDEX_RENAMED */ || this._type === 4 /* Status.INDEX_COPIED */)) {
    ...
    getIconPath(theme) {
        switch (this.type) {
            ...
            case 15 /* Status.BOTH_DELETED */: return Resource.Icons[theme].Conflict;
            case 10 /* Status.ADDED_BY_US */: return Resource.Icons[theme].Conflict;
            case 13 /* Status.DELETED_BY_THEM */: return Resource.Icons[theme].Conflict;
            case 11 /* Status.ADDED_BY_THEM */: return Resource.Icons[theme].Conflict;
            case 12 /* Status.DELETED_BY_US */: return Resource.Icons[theme].Conflict;
            case 14 /* Status.BOTH_ADDED */: return Resource.Icons[theme].Conflict;
            case 16 /* Status.BOTH_MODIFIED */: return Resource.Icons[theme].Conflict;
...
function isReadOnly(operation) {
    switch (operation) {
        case "Blame" /* Operation.Blame */:
        case "CheckIgnore" /* Operation.CheckIgnore */:
        case "Diff" /* Operation.Diff */:
        case "GetTracking" /* Operation.FindTrackingBranches */:
        case "GetBranch" /* Operation.GetBranch */:
        case "GetCommitTemplate" /* Operation.GetCommitTemplate */:
        case "GetObjectDetails" /* Operation.GetObjectDetails */:
        case "Log" /* Operation.Log */:
        case "LogFile" /* Operation.LogFile */:
        case "MergeBase" /* Operation.MergeBase */:
        case "Show" /* Operation.Show */:
            return true;
        default:
            return false;
    }
...
class FileEventLogger {
    constructor(onWorkspaceWorkingTreeFileChange, onDotGitFileChange, outputChannelLogger) {
...
class DotGitWatcher {
    constructor(repository, outputChannelLogger) {
        this.repository = repository;
        this.outputChannelLogger = outputChannelLogger;
    ...
    updateTransientWatchers() {
        ...
        const upstreamPath = path.join((_a = this.repository.dotGit.commonPath) !== null && _a !== void 0 ? _a : this.repository.dotGit.path, 'refs', 'remotes', remote, name);
        try {
...
class ResourceCommandResolver {
    ...
    resolveChangeCommand(resource) {
        ...
        if (!resource.leftUri) {
            const bothModified = resource.type === 16 /* Status.BOTH_MODIFIED */;
            if (resource.rightUri && bothModified && vscode_1.workspace.getConfiguration('git').get('experimental.mergeEditor', false)) {
                return {
...
class Repository {
    constructor(repository, pushErrorHandlerRegistry, remoteSourcePublisherRegistry, globalState, outputChannelLogger, telemetryReporter) {
        this.repository = repository;
        ...
        (0, util_1.filterEvent)(vscode_1.workspace.onDidChangeConfiguration, e => e.affectsConfiguration('git.branchSortOrder', root)
            ...
            || e.affectsConfiguration('git.showUnpublishedCommitsButton', root))(this.updateModelState, this, this.disposables);
        ...
        const onSuccessfulPush = (0, util_1.filterEvent)(this.onDidRunOperation, e => e.operation === "Push" /* Operation.Push */ && !e.error);
        onSuccessfulPush(() => {
        ...
        const actionButton = new actionButton_1.ActionButtonCommand(this);
    ...
    get headShortName() {
        ...
        const tag = this.refs.filter(iref => iref.type === 2 /* RefType.Tag */ && iref.commit === HEAD.commit)[0];
    ...
    getConfigs() {
        return this.run("Config" /* Operation.Config */, () => this.repository.getConfigs('local'));
    }
    getConfig(key) {
        return this.run("Config" /* Operation.Config */, () => this.repository.config('local', key));
    }
    getGlobalConfig(key) {
        return this.run("Config" /* Operation.Config */, () => this.repository.config('global', key));
    }
    setConfig(key, value) {
        return this.run("Config" /* Operation.Config */, () => this.repository.config('local', key, value));
    }
    log(options) {
        return this.run("Log" /* Operation.Log */, () => this.repository.log(options));
    }
    logFile(uri, options) {
        // TODO: This probably needs per-uri granularity
        return this.run("LogFile" /* Operation.LogFile */, () => this.repository.logFile(uri, options));
    }
    async status() {
        await this.run("Status" /* Operation.Status */);
    }
    diff(cached) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diff(cached));
    }
    diffWithHEAD(path) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffWithHEAD(path));
    }
    diffWith(ref, path) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffWith(ref, path));
    }
    diffIndexWithHEAD(path) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffIndexWithHEAD(path));
    }
    diffIndexWith(ref, path) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffIndexWith(ref, path));
    }
    diffBlobs(object1, object2) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffBlobs(object1, object2));
    }
    diffBetween(ref1, ref2, path) {
        return this.run("Diff" /* Operation.Diff */, () => this.repository.diffBetween(ref1, ref2, path));
    }
    getMergeBase(ref1, ref2) {
        return this.run("MergeBase" /* Operation.MergeBase */, () => this.repository.getMergeBase(ref1, ref2));
    }
    async hashObject(data) {
        return this.run("HashObject" /* Operation.HashObject */, () => this.repository.hashObject(data));
    }
    async add(resources, opts) {
        await this.run("Add" /* Operation.Add */, async () => {
            await this.repository.add(resources.map(r => r.fsPath), opts);
            this.closeDiffEditors([], [...resources.map(r => r.fsPath)]);
        });
    }
    async rm(resources) {
        await this.run("Remove" /* Operation.Remove */, () => this.repository.rm(resources.map(r => r.fsPath)));
    }
    async stage(resource, contents) {
        const path = (0, util_1.relativePath)(this.repository.root, resource.fsPath).replace(/\\/g, '/');
        await this.run("Stage" /* Operation.Stage */, async () => {
    ...
    async revert(resources) {
        await this.run("RevertFiles" /* Operation.RevertFiles */, async () => {
    ...
    async commit(message, opts = Object.create(null)) {
        ...
        if (this.rebaseCommit) {
            await this.run("RebaseContinue" /* Operation.RebaseContinue */, async () => {
                if (opts.all) {
            ...
            await this.run("Commit" /* Operation.Commit */, async () => {
                if (opts.all) {
                    const addOpts = opts.all === 'tracked' ? { update: true } : {};
                    await this.repository.add([], addOpts);
                }
    ...
    async clean(resources) {
        await this.run("Clean" /* Operation.Clean */, async () => {
            const toClean = [];
            const toCheckout = [];
    ...
    closeDiffEditors(indexResources, workingTreeResources, ignoreSetting = false) {
        ...
        for (const tab of vscode_1.window.tabGroups.all.map(g => g.tabs).flat()) {
            ...
            if (input instanceof vscode_1.TabInputTextDiff || input instanceof vscode_1.TabInputNotebookDiff) {
                if (input.modified.scheme === 'git' && indexResources.some(r => (0, util_1.pathEquals)(r, input.modified.fsPath))) {
                    // Index
                    diffEditorTabsToClose.push(tab);
                }
                if (input.modified.scheme === 'file' && input.original.scheme === 'git' && workingTreeResources.some(r => (0, util_1.pathEquals)(r, input.modified.fsPath))) {
                    // Working Tree
                    diffEditorTabsToClose.push(tab);
                }
            }
    ...
    async branch(name, _checkout, _ref) {
        await this.run("Branch" /* Operation.Branch */, () => this.repository.branch(name, _checkout, _ref));
    }
    async deleteBranch(name, force) {
        await this.run("DeleteBranch" /* Operation.DeleteBranch */, () => this.repository.deleteBranch(name, force));
    }
    async renameBranch(name) {
        await this.run("RenameBranch" /* Operation.RenameBranch */, () => this.repository.renameBranch(name));
    }
    async cherryPick(commitHash) {
        await this.run("CherryPick" /* Operation.CherryPick */, () => this.repository.cherryPick(commitHash));
    }
    async move(from, to) {
        await this.run("Move" /* Operation.Move */, () => this.repository.move(from, to));
    }
    async getBranch(name) {
        return await this.run("GetBranch" /* Operation.GetBranch */, () => this.repository.getBranch(name));
    }
    async getBranches(query) {
        return await this.run("GetBranches" /* Operation.GetBranches */, () => this.repository.getBranches(query));
    }
    async setBranchUpstream(name, upstream) {
        await this.run("SetBranchUpstream" /* Operation.SetBranchUpstream */, () => this.repository.setBranchUpstream(name, upstream));
    }
    async merge(ref) {
        await this.run("Merge" /* Operation.Merge */, () => this.repository.merge(ref));
    }
    async rebase(branch) {
        await this.run("Rebase" /* Operation.Rebase */, () => this.repository.rebase(branch));
    }
    async tag(name, message) {
        await this.run("Tag" /* Operation.Tag */, () => this.repository.tag(name, message));
    }
    async deleteTag(name) {
        await this.run("DeleteTag" /* Operation.DeleteTag */, () => this.repository.deleteTag(name));
    }
    async checkout(treeish, opts) {
        await this.run("Checkout" /* Operation.Checkout */, () => this.repository.checkout(treeish, [], opts));
    }
    async checkoutTracking(treeish, opts = {}) {
        await this.run("CheckoutTracking" /* Operation.CheckoutTracking */, () => this.repository.checkout(treeish, [], { ...opts, track: true }));
    }
    async findTrackingBranches(upstreamRef) {
        return await this.run("GetTracking" /* Operation.FindTrackingBranches */, () => this.repository.findTrackingBranches(upstreamRef));
    }
    async getCommit(ref) {
        return await this.repository.getCommit(ref);
    }
    async reset(treeish, hard) {
        await this.run("Reset" /* Operation.Reset */, () => this.repository.reset(treeish, hard));
    }
    async deleteRef(ref) {
        await this.run("DeleteRef" /* Operation.DeleteRef */, () => this.repository.deleteRef(ref));
    }
    async addRemote(name, url) {
        await this.run("Remote" /* Operation.Remote */, () => this.repository.addRemote(name, url));
    }
    async removeRemote(name) {
        await this.run("Remote" /* Operation.Remote */, () => this.repository.removeRemote(name));
    }
    async renameRemote(name, newName) {
        await this.run("Remote" /* Operation.Remote */, () => this.repository.renameRemote(name, newName));
    }
    ...
    async fetchAll(cancellationToken) {
    ...
    async _fetch(options = {}) {
        ...
        await this.run("Fetch" /* Operation.Fetch */, async () => this.repository.fetch(options));
    }
    ...
    async pullFrom(rebase, remote, branch, unshallow) {
        await this.run("Pull" /* Operation.Pull */, async () => {
    ...
    async push(head, forcePushMode) {
        ...
        await this.run("Push" /* Operation.Push */, () => this._push(remote, branch, undefined, undefined, forcePushMode));
    }
    async pushTo(remote, name, setUpstream = false, forcePushMode) {
        await this.run("Push" /* Operation.Push */, () => this._push(remote, name, setUpstream, undefined, forcePushMode));
    }
    async pushFollowTags(remote, forcePushMode) {
        await this.run("Push" /* Operation.Push */, () => this._push(remote, undefined, false, true, forcePushMode));
    }
    async pushTags(remote, forcePushMode) {
        await this.run("Push" /* Operation.Push */, () => this._push(remote, undefined, false, false, forcePushMode, true));
    }
    async blame(path) {
        return await this.run("Blame" /* Operation.Blame */, () => this.repository.blame(path));
    }
    sync(head) {
    ...
    async _sync(head, rebase) {
        ...
        await this.run("Sync" /* Operation.Sync */, async () => {
            await this.maybeAutoStash(async () => {
                const config = vscode_1.workspace.getConfiguration('git', vscode_1.Uri.file(this.root));
                const fetchOnPull = config.get('fetchOnPull');
                const tags = config.get('pullTags');
    ...
    async checkIfMaybeRebased(currentBranch) {
        ...
        const maybeRebased = await this.run("Log" /* Operation.Log */, async () => {
            try {
    ...
    async show(ref, filePath) {
        return await this.run("Show" /* Operation.Show */, async () => {
    ...
    async buffer(ref, filePath) {
        return this.run("Show" /* Operation.Show */, () => {
            const path = (0, util_1.relativePath)(this.repository.root, filePath).replace(/\\/g, '/');
            return this.repository.buffer(`${ref}:${path}`);
        });
    }
    getObjectDetails(ref, filePath) {
        return this.run("GetObjectDetails" /* Operation.GetObjectDetails */, () => this.repository.getObjectDetails(ref, filePath));
    }
    detectObjectType(object) {
        return this.run("Show" /* Operation.Show */, () => this.repository.detectObjectType(object));
    }
    async apply(patch, reverse) {
        return await this.run("Apply" /* Operation.Apply */, () => this.repository.apply(patch, reverse));
    }
    async getStashes() {
    ...
    async createStash(message, includeUntracked) {
        ...
        return await this.run("Stash" /* Operation.Stash */, async () => {
            this.repository.createStash(message, includeUntracked);
            this.closeDiffEditors(indexResources, workingGroupResources);
        });
    }
    async popStash(index) {
        return await this.run("Stash" /* Operation.Stash */, () => this.repository.popStash(index));
    }
    async dropStash(index) {
        return await this.run("Stash" /* Operation.Stash */, () => this.repository.dropStash(index));
    }
    async applyStash(index) {
        return await this.run("Stash" /* Operation.Stash */, () => this.repository.applyStash(index));
    }
    async getCommitTemplate() {
        return await this.run("GetCommitTemplate" /* Operation.GetCommitTemplate */, async () => this.repository.getCommitTemplate());
    }
    async ignore(files) {
        return await this.run("Ignore" /* Operation.Ignore */, async () => {
    ...
    async rebaseAbort() {
        await this.run("RebaseAbort" /* Operation.RebaseAbort */, async () => await this.repository.rebaseAbort());
    }
    checkIgnore(filePaths) {
        return this.run("CheckIgnore" /* Operation.CheckIgnore */, () => {
        ...
        while (true) {
            try {
                ...
                const shouldRetry = attempt <= 10 && ((err.gitErrorCode === "RepositoryIsLocked" /* GitErrorCodes.RepositoryIsLocked */)
                    || ((operation === "Pull" /* Operation.Pull */ || operation === "Sync" /* Operation.Sync */ || operation === "Fetch" /* Operation.Fetch */) && (err.gitErrorCode === "CantLockRef" /* GitErrorCodes.CantLockRef */ || err.gitErrorCode === "CantRebaseMultipleBranches" /* GitErrorCodes.CantRebaseMultipleBranches */)));
                if (shouldRetry) {
                    // quatratic backoff
        ...
        status.forEach(raw => {
            ...
            switch (raw.x + raw.y) {
                case '??': switch (untrackedChanges) {
                    case 'mixed': return workingTree.push(new Resource(this.resourceCommandResolver, 2 /* ResourceGroupType.WorkingTree */, uri, 7 /* Status.UNTRACKED */, useIcons));
                    case 'separate': return untracked.push(new Resource(this.resourceCommandResolver, 3 /* ResourceGroupType.Untracked */, uri, 7 /* Status.UNTRACKED */, useIcons));
                    default: return undefined;
                }
                case '!!': switch (untrackedChanges) {
                    case 'mixed': return workingTree.push(new Resource(this.resourceCommandResolver, 2 /* ResourceGroupType.WorkingTree */, uri, 8 /* Status.IGNORED */, useIcons));
                    case 'separate': return untracked.push(new Resource(this.resourceCommandResolver, 3 /* ResourceGroupType.Untracked */, uri, 8 /* Status.IGNORED */, useIcons));
                    default: return undefined;
                }
                case 'DD': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 15 /* Status.BOTH_DELETED */, useIcons));
                case 'AU': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 10 /* Status.ADDED_BY_US */, useIcons));
                case 'UD': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 13 /* Status.DELETED_BY_THEM */, useIcons));
                case 'UA': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 11 /* Status.ADDED_BY_THEM */, useIcons));
                case 'DU': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 12 /* Status.DELETED_BY_US */, useIcons));
                case 'AA': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 14 /* Status.BOTH_ADDED */, useIcons));
                case 'UU': return merge.push(new Resource(this.resourceCommandResolver, 0 /* ResourceGroupType.Merge */, uri, 16 /* Status.BOTH_MODIFIED */, useIcons));
            }
            switch (raw.x) {
                case 'M':
                    index.push(new Resource(this.resourceCommandResolver, 1 /* ResourceGroupType.Index */, uri, 0 /* Status.INDEX_MODIFIED */, useIcons));
                    break;
                case 'A':
                    index.push(new Resource(this.resourceCommandResolver, 1 /* ResourceGroupType.Index */, uri, 1 /* Status.INDEX_ADDED */, useIcons));
                    break;
                case 'D':
                    index.push(new Resource(this.resourceCommandResolver, 1 /* ResourceGroupType.Index */, uri, 2 /* Status.INDEX_DELETED */, useIcons));
                    break;
                case 'R':
                    index.push(new Resource(this.resourceCommandResolver, 1 /* ResourceGroupType.Index */, uri, 3 /* Status.INDEX_RENAMED */, useIcons, renameUri));
                    break;
                case 'C':
                    index.push(new Resource(this.resourceCommandResolver, 1 /* ResourceGroupType.Index */, uri, 4 /* Status.INDEX_COPIED */, useIcons, renameUri));
                    break;
            }
            switch (raw.y) {
                case 'M':
                    workingTree.push(new Resource(this.resourceCommandResolver, 2 /* ResourceGroupType.WorkingTree */, uri, 5 /* Status.MODIFIED */, useIcons, renameUri));
                    break;
                case 'D':
                    workingTree.push(new Resource(this.resourceCommandResolver, 2 /* ResourceGroupType.WorkingTree */, uri, 6 /* Status.DELETED */, useIcons, renameUri));
                    break;
                case 'A':
                    workingTree.push(new Resource(this.resourceCommandResolver, 2 /* ResourceGroupType.WorkingTree */, uri, 9 /* Status.INTENT_TO_ADD */, useIcons, renameUri));
                    break;
            }
            return undefined;
        });
